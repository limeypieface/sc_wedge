...
interface AddLineModalProps {
  ...
  /** Display mode: "full" shows catalog/requisition search, "mvp" shows simple direct entry */
  mode?: Mode
}
...
export function AddLineModal({ isOpen, onClose, onAdd, vendorId, nextLineNumber, mode = "full" }: AddLineModalProps) {
  ...
  // MVP direct entry fields
  const [mvpSku, setMvpSku] = useState("")
  const [mvpName, setMvpName] = useState("")
  const [mvpDescription, setMvpDescription] = useState("")
  const [mvpUom, setMvpUom] = useState("EA")
  ...
  const handleSelectCatalogItem = (item: CatalogItem) => {
    ...
    setTaxCode((item.taxCode as "STANDARD" | "REDUCED" | "EXEMPT") || "STANDARD")
    setDiscountPercent("0")
    setProjectCode("")
  ...
  const handleSelectRequisition = (req: OpenRequisitionLine) => {
    ...
    setTaxCode((catalogItem?.taxCode as "STANDARD" | "REDUCED" | "EXEMPT") || "STANDARD")
    setDiscountPercent("0")
  ...
  // Can add in MVP mode (simple validation)
  const canAddMVP = useMemo(() => {
    if (!isMVP) return false
    return mvpSku.trim() !== "" && mvpName.trim() !== "" && parseFloat(quantity) > 0 && parseFloat(unitPrice) > 0
  }, [isMVP, mvpSku, mvpName, quantity, unitPrice])

  // Can add if there's a selection and validation passes (or service is valid, or MVP mode)
  const canAdd = isMVP ? canAddMVP : (sourceMode === "service" ? canAddService : (selectedItem && validationResult?.isValid))

  const handleAdd = () => {
    // Handle MVP direct entry
    if (isMVP) {
      const qty = parseFloat(quantity) || 0
      const price = parseFloat(unitPrice) || 0
      const lineTotal = qty * price
      const taxRateVal = taxCode === "STANDARD" ? 0.0825 : taxCode === "REDUCED" ? 0.04 : 0

      const newLine: NewLineData = {
        lineNumber: nextLineNumber,
        sku: mvpSku.trim(),
        name: mvpName.trim(),
        description: mvpDescription.trim(),
        quantity: qty,
        unitOfMeasure: mvpUom || "EA",
        unitPrice: price,
        lineTotal,
        projectCode: "",
        commodityCode: "",
    ...
    const newLine: NewLineData = {
      ...
      need: selectedRequisition ? {
        moNumber: selectedRequisition.moNumber || selectedRequisition.projectCode,
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col">
        ...
        {/* MVP Mode: Simple direct entry form */}
        {isMVP ? (
          <div className="space-y-4 py-2">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label className="text-[11px] text-muted-foreground mb-1 block">SKU / Part Number</Label>
                <Input
                  value={mvpSku}
                  onChange={(e) => setMvpSku(e.target.value)}
                  placeholder="e.g., PART-001"
                  className="h-8 text-sm"
                />
              </div>
              <div>
                <Label className="text-[11px] text-muted-foreground mb-1 block">Unit of Measure</Label>
                <Input
                  value={mvpUom}
                  onChange={(e) => setMvpUom(e.target.value)}
                  placeholder="EA"
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                <Label className="text-[11px] text-muted-foreground mb-1 block">Tax Rate %</Label>
                <Input
                  type="number"
                  step="0.01"
                  value={taxCode === "STANDARD" ? "8.25" : taxCode === "REDUCED" ? "4" : "0"}
                  onChange={(e) => {
                    const rate = parseFloat(e.target.value) || 0
                    if (rate === 0) setTaxCode("EXEMPT")
                    else if (rate < 5) setTaxCode("REDUCED")
                    else setTaxCode("STANDARD")
                  }}
                  className="h-8 text-sm w-20"
                />
              </div>
              ...
              <div className="pt-2 border-t text-sm">
                <div className="flex justify-between text-muted-foreground">
                  ...
                  <span className="tabular-nums font-medium text-foreground">
                    ${((parseFloat(quantity) || 0) * (parseFloat(unitPrice) || 0)).toLocaleString('en-US', { minimumFractionDigits: 2 })}
                  </span>
                </div>
              </div>
            )}
