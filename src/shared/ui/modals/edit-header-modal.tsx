...
export function EditHeaderModal({ isOpen, onClose, onSave, header, headerCharges }: EditHeaderModalProps) {
  ...
  useEffect(() => {
    if (header && isOpen) {
      setShippingMethod(header.shippingMethod || "")
      setShippingTerms(header.shipping?.instructions || "")
      setPaymentTerms(header.paymentTerms || "")
  ...
  const fieldChanges = useMemo(() => {
    ...
    if (shippingTerms !== (header.shipping?.instructions || "")) {
      result.push({ field: "shippingTerms", oldValue: header.shipping?.instructions || "", newValue: shippingTerms, label: "Shipping Terms" })
    }
