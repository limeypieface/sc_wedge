...
import { getLineItemModalConfig, type LineItemModalVariant } from "@/shared/ui/line-items"
...
interface LineItemModalProps {
  ...
  poNumber?: string
  /** Generic order number (PO or SO) */
  orderNumber?: string
  /** Variant for PO vs SO terminology */
...
export function LineItemModal({
  ...
  // Support both legacy poNumber and new orderNumber props
  const effectiveOrderNumber = orderNumber || poNumber || "PO-2026-00142"
  const config = getLineItemModalConfig(variant)
  ...
  // Get order data (works for both PO and SO)
  const orderData = getPOData(effectiveOrderNumber)
  const shipments = orderData?.shipments || []
  const invoices = orderData?.invoices || []

  // Get all issues for this order
  const allIssues = detectPOIssuesForPO(effectiveOrderNumber)
  ...
  const lineNeeds = peggedNeeds.filter(n => n.lineNumber === item.lineNumber)
  ...
  // Tabs - Needs tab only shown for PO variant
  const tabs = config.showNeedsTab ? [
        ...
        <div className="pt-4">
          ...
          {activeTab === "overview" && (
            <div className="space-y-6">
              ...
              {/* Quantity Flow */}
              <div>
                <h4 className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-3">{config.labels.quantityFlowTitle}</h4>
                <div className="flex items-center gap-2">
                  {config.quantityFlow.map((step, i, arr) => (
                    { label: step.label, value: step.getValue(item) }
                  )).map((stat, i, arr) => (
              ...
              {/* Invoices & Payments */}
              <div>
                <h4 className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-3">{config.labels.invoicesTitle}</h4>
                ...
                <div className="grid grid-cols-4 gap-3 mb-4">
                  ...
                  <div className="bg-muted/30 rounded-lg p-3 text-center">
                    ...
                    <p className="text-xs text-muted-foreground">{config.labels.paidAmountLabel}</p>
                  </div>
                </div>
                            ...
                            <tr key={need.id} className={cn("hover:bg-muted/30 transition-colors", atRisk && "bg-amber-50/30")}>
                              <td className="py-2.5 px-3">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="font-medium font-mono">{need.referenceNumber}</span>
                                </div>
                              </td>
                              <td className="py-2.5 px-3 text-muted-foreground">{need.projectName}</td>
                              <td className="py-2.5 px-3 text-right tabular-nums font-medium">{need.qtyNeeded}</td>
                              <td className="py-2.5 px-3">
                                <div className="flex items-center gap-1.5">
                                  <Calendar className="w-3.5 h-3.5 text-muted-foreground" />
                                  <span>{need.needDate}</span>
                                </div>
                              </td>
