...
export function TasksTab() {
  ...
  const toggleComplete = (issue: POIssue) => {
    if (completedTaskIds.has(issue.id)) {
      ...
      setCompletedTasks((prev) => [
        ...prev,
        { id: issue.id, title: issue.title || issue.description, completedDate: "Jan 27, 2026" },
      ])
    }
  }
  ...
  const getActionButton = (issue: POIssue) => {
    ...
    const config = issue.suggestedAction ? actionConfig[issue.suggestedAction as IssueActionType] : null
