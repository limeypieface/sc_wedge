 ...
 * Supports role-based approvers, approval chains, and threshold-based rules.
 *
 * Workflow Overview:
 * 1. Buyer makes changes to PO (creates draft revision)
 ...
 * 5. Once approved, revision can be sent to vendor
 */
...
export function calculateCostDelta(
  ...
  const exceedsPercentage = Math.abs(percentChange) >= config.percentageThreshold;
